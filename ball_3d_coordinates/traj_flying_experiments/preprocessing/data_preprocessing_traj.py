...
class TmpTrajPreprocessor(object):
	def __init__(self, input_trace):
		super(TmpTrajPreprocessor, self).__init__()
		self.input_trace = input_trace

	def fit_transform(self, X, y, noise=False, lstm=True):
		...
		X, y = self.generate_data(X, y, train_list_complete, train_list, lstm)
	...
	def generate_data(self, features, labels, train_list_complete, train_list, lstm):
		data_x = []
		data_y = []
